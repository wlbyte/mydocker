...
func setupMount() error {
	...
	if err := unix.Mount("proc", "/proc", "proc", uintptr(defaultMountFlags), ""); err != nil {
		return fmt.Errorf(errFormat, "unix.Mount /proc", err)
	}
	if err := unix.Mount("tmpfs", "/dev", "tmpfs", unix.MS_NOSUID|unix.MS_STRICTATIME, "mode=755"); err != nil {
		return fmt.Errorf(errFormat, "unix.Mount /dev", err)
	}
